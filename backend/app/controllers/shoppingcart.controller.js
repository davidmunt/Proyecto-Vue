...
const addArticle = asyncHandler(async (req, res) => {
  ...
  if (!cart) {
    ...
    const existingArticleIndex = cart.articles.findIndex(
      (a) => a.refArticulo === article.refArticulo
    );
...
const sumArticle = asyncHandler(async (req, res) => {
  const { userId, refArticulo } = req.params;
  if (!refArticulo) {
    return res
      .status(400)
      .json({ message: "Se requiere refArticulo para sumar la cantidad del artuculo" });
  }
  if (!userId) {
    return res.status(400).json({ message: "Se requiere userId para eliminar" });
  }
  const cart = await ShoppingCart.findOne({ userId, purchased: false }).exec();
  if (!cart) {
    return res.status(404).json({ message: "Carrito activo no encontrado para este usuario" });
  }
  const article = cart.articles.find((a) => a.refArticulo === refArticulo);
  if (!article) {
    return res.status(404).json({ message: "Articulo no encontrado en el carrito" });
  }
  article.cantidad += 1;
  cart.totalPrice = cart.articles.reduce((acc, a) => acc + a.cantidad * a.precio, 0);
  await cart.save();
...
const restArticle = asyncHandler(async (req, res) => {
  const { userId, refArticulo } = req.params;
  if (!refArticulo) {
    return res
      .status(400)
...
const checkout = asyncHandler(async (req, res) => {
  ...
  if (!cart || cart.articles.length === 0) {
    return res
      .status(400)
      .json({ message: "No hay artículos en el carrito para finalizar la compra" });
  }
  cart.purchased = true;
...
const getPurchasesUser = asyncHandler(async (req, res) => {
  const { userId } = req.params;
  const purchases = await ShoppingCart.find({ userId, purchased: true })
    .sort({ purchaseDate: -1 })
    .exec();
